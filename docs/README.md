...
14. [Generic table models](14-generic-table-models.md) - работа с таблицами и связями без объявления PHP-классов моделей.

## Фасады, value objects и результаты

`ActiveRecord` - главный публичный фасад модели и вход в generic table models через `fromTable()`. Один объект может быть ленивым запросом, коллекцией строк или row-bound моделью конкретной строки. Это сделано специально: можно писать коротко, а библиотека догружает данные в момент чтения.
