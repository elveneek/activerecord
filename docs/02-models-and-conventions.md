...
Сбросить сортировку для конкретного запроса можно тремя способами, от самого короткого:

```php
// 1. Пустой orderBy() — самый короткий вариант (удобно для скорости или single-item запросов)
Product::orderBy('');

// 2. Явный withoutOrder() прямо на модели
Product::all()->withoutOrder();

// 3. Через низкоуровневый builder, если нужно дальше собирать запрос руками
$query = Product::all()->toQuery()->withoutOrder();
$products = Product::fromQuery($query);
```

`orderBy('')` и `withoutOrder()` полностью убирают `ORDER BY` (включая `defaultOrder`), что полезно для быстрых запросов или когда нужна одна запись без лишней сортировки. Обычный `orderBy('col')` не заменяет `defaultOrder`, а добавляет к нему — если нужно именно заменить, используйте сброс выше.
